...
web-genius-agent/
│
├── config.json             ← הגדרות המותג/החנות (מטבע, צבעים, אנליטיקס...) — זה מה שמחליפים בין לקוחות
│
...
│
└── docs/                   ← מדריכים ומסמכים תומכים (כולל config-guide.md)
```
...
כל התוכן שהיה בעבר קשיח (מטבע, סף משלוח חינם, ימי החזרה, צבעי מותג, פונטים, מזהי GA4/GTM/Pixel) יושב עכשיו במקום אחד — `config.json`. כדי לבנות אתר/חנות חדשה:

1. שכפלו את `config.json` ומלאו את ערכי הלקוח החדש.
2. אל תיגעו ב-`skills/` או ב-`templates/` — הם קוראים ערכים מה-config דרך placeholders בפורמט `{{ config.<path> }}`.
3. פרטים מלאים: [`docs/config-guide.md`](docs/config-guide.md).
